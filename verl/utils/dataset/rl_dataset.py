...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.image_pres_key = self.config.get("image_pres_key", "images_pres")
        self.image_abl_key = self.config.get("image_abl_key", "images_abl")
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if self.image_pres_key in row_dict and row_dict.get(self.image_pres_key, None) is not None:
                images_pres = [
                    process_image(image) for image in row_dict.pop(self.image_pres_key)
                ]
                row_dict["pres_multi_modal_data"] = {"image": images_pres}

                pres_multimodal_inputs = self.processor.image_processor(
                    images=images_pres, return_tensors="pt"
                )
                row_dict["pres_multi_modal_inputs"] = dict(pres_multimodal_inputs)

            if self.image_abl_key in row_dict and row_dict.get(self.image_abl_key, None) is not None:
                images_abl = [
                    process_image(image) for image in row_dict.pop(self.image_abl_key)
                ]
                row_dict["abl_multi_modal_data"] = {"image": images_abl}

                abl_multimodal_inputs = self.processor.image_processor(
                    images=images_abl, return_tensors="pt"
                )
                row_dict["abl_multi_modal_inputs"] = dict(abl_multimodal_inputs)
